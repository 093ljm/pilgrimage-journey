...
**建議方法（已驗證）：**
1. 用戶上傳圖片到 GitHub repo 的 `public/images/`
...
用戶推圖到 GitHub  →  AI 先 curl 下載到本地  →  ✅ 之後推送會一起帶上去
```

**給 AI 的鐵則：**
1. 用戶說「圖片上傳好了」→ **立刻 curl 下載**，不要拖到之後
2. 下載完 `ls -lh` 確認檔案存在且大小正常，再做任何 git 操作
3. **在確認圖片已進入本地 `public/images/` 之前，絕對不要 force push**
4. 主動提醒用戶：「上傳後請馬上告訴我」，不要讓用戶默默上傳

**其他方式：**
- Same 檔案總管上傳：2026-03 曾失效，但**可能已修復，值得先試一次**
  （若檔案出現在 `public/images/` 就直接可用，最省事）
- 拖放到聊天視窗：只能查看，無法存檔
