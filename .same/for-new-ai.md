...
- ✅ 常見問題 (/faq) — 共 9 題
- ✅ 法師開示 (/teachings)
- ✅ **活動資訊 (/events/info)** — 觀音三會、社群連結、活動回顧（檳城篇）
- ✅ **行前準備 (/preparation)** — 身／心／事三段準備 + 儀軌節奏
- ✅ **生命故事 (/events/stories)** — 筱喻師姐〈27年，從泥巴路開始〉含影片
- ✅ 伺服器穩定性修復

### 📌 明天（2026-08-12）
用戶說會一併回覆問題。**請先讀 `.same/todos.md` 最上方的「📌 明天的起點」**，
裡面列了三個等回覆的問題，以及素材交付流程。

### 待完成
- [ ] 三個筆記頁面（人物／寶物／地點）— 需用戶提供素材
- [ ] ⭐ 管理後台 Decap CMS（已提醒，等用戶決定時機）
- [ ] 用戶還有一篇活動回顧文章未整理好
- [ ] 影片改用 YouTube（用戶會給連結，可讓倉庫瘦身 38MB）
- [ ] 部署到 Netlify

### 資料檔位置（內容與程式已分離）
- `src/data/events.ts` — 年度朝山日、社群連結、活動回顧（含全文）
- `src/data/stories.ts` — 生命故事（含全文、影片設定）
- `src/data/preparation.ts` — 行前準備

### 用戶素材倉庫（★ 重要）
用戶把素材放在**另一個倉庫**，避免被我的 force push 覆蓋：
`https://github.com/goodseedsinmind1111-web/pilgrimage-journey-tara`
（我只讀取、從不推送到那裡）
